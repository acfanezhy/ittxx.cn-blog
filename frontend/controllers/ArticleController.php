...
class ArticleController extends Controller
{
    ...
    public function actionIndex($cat = '')
    {
        ...
        if ($cat != '' && $cat != 'index') {
		 ...
		 $params = yii::$app->getRequest()->getQueryParams();  //URL传递的参数
        //var_dump($params);
        $newparams = array_slice($params,1);  //去掉第一个参数cat
        //print_r($newparams);
        if(in_array('ctype', array_keys($newparams))) $where['ctype']= $newparams['ctype'];
        ...
        $catName= Category::findOne(['alias' => $cat])['name'];
        return $this->render('index', [
            'dataProvider' => $dataProvider,
            'type' => ( !empty($cat) ? yii::t('frontend', 'Category {cat} articles', ['cat'=>'<span class=isearch>'.$catName.'</span>']) : yii::t('frontend', 'Latest Articles') ),
            'cat'=>$cat
        ]);
    }
    ...
    public function actionViewAjax($id)
    {
        ...
        return [
            ...
            'shareCount' => (int)$model->getArticleShareCount(),
            'scanCount' => $model->scan_count,
            'commentCount' => Comment::getCommentCount($model->id),
        ];
    }
    ...
    public function actionComment()
    {
        if (yii::$app->getRequest()->getIsPost()) {
            ...
            if ($commentModel->load(yii::$app->getRequest()->post()) && $commentModel->save()) {
                //$avatar = User::findByUsername($commentModel->nickname)['avatar'];
                //if(!$avatar){
                    $avatar = '/admin/static/img/default.png';
                //}
    ...
    {
        $aid = yii::$app->getRequest()->post("aid");
        $model = new ArticleMetaShare();
        $model->setShare($aid);
        return $model->getShareCount($aid);
