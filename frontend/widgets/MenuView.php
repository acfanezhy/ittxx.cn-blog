...
class MenuView extends \yii\base\Widget
{
    ...
    public $subTemplate = "<ul class=\"sub-menu\">{lis}</ul>";

    public $subLitemplate = "<li id=\"menu-item-{menu_id}\" class=\"menu-item menu-item-type-taxonomy menu-item-object-category {current_menu_class} menu-item-{menu_id}\"><a href=\"{url}\" target='{target}'><i class='fa fa-angle-right'></i>{title}</a></li>";
    ...
    public function run()
    {
        ...
        foreach ($menus as $key => $menu) {
            if ($menu['parent_id'] == 0) {
                ...
                $url=explode('?', yii::$app->getRequest()->getUrl())[0];
                //var_dump($menu['url']);
                if($menu['url']== $url){
                    $thisid=$menu['parent_id'];
                    echo "<style>#menu-item-{$thisid}>a,#menu-item-{$thisid}>a:hover{background:#1ABC9C; }</style>";
                }
            }
        }
